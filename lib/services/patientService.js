...
export const patientService = {
  ...
  async updatePatient(id, { name, gender, contact, medical_history, medications, visit_history, birthday }) {
    // Validate required fields
    if (!name || !gender || !birthday) {
      throw new Error("name, gender, and birthday are required");
    }
    ...
    return Patient.findByIdAndUpdate(
      id,
      { name, gender, contact, medical_history, medications, visit_history, birthday },
      { new: true }
    );
  },
