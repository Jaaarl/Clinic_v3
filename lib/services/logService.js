...
/**
 * Log Service - All logging-related business logic
 */
export const logService = {
  /**
   * Get inventory logs with filtering and pagination
   * @param {object} options - Filter options
   * @returns {Promise<{ logs: InventoryLog[], pagination: object }>}
   */
  ...
  async getSalesData({
    ...
    if (expirationStartDate || expirationEndDate) {
      ...
      if (expirationStartDate) {
        matchFilter["previousData.expirationDate"].$gte = new Date(expirationStartDate);
      }
      if (expirationEndDate) {
        matchFilter["previousData.expirationDate"].$lte = new Date(expirationEndDate);
      }
    }
    ...
    const expiredResult = expiredItemsSales[0] || {
      totalQuantitySold: 0,
      totalRevenue: 0,
      totalTransactions: 0,
    };
