...
export default function AppointmentsPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/patient?search=${encodeURIComponent(patientSearch)}&limit=20`);
            ...
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  Time {!availableSlots.length && "(select doctor & date first)"}
                </label>
                <select
                  name="scheduledTime"
                ...
                {appointments.map((apt) => (
                  <tr key={apt._id}>
                    ...
                    <td className="px-4 py-3">
                      {apt.doctorId?.name ? `Dr. ${apt.doctorId.name}` : "Unassigned"}
                    </td>
